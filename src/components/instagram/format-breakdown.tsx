 ...
 * arrives sorted best-engagement first, so the first card is the "champion"
 * and gets a highlight ring + badge.
 */
export function FormatBreakdown({ formats }: { formats: FormatPerf[] }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {formats.map((f, i) => (
        <Card key={f.type} className={cn("p-4", i === 0 && "ring-1 ring-primary/40")}>
